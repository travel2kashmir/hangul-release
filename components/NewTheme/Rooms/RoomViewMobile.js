...
function RoomViewMobile({ rooms, selectedRoom, setSelectedRoom, showRoom, setShowRoom, lang,currency,cookie }) {
    return (
                            ...
                            <div className={`text-center pb-5 md:py-5 lg:py-10 md:rounded-md ${selectedRoom?.room_id === room?.room_id ? `${`md:shadow-xl md:bg-slate-200 lg:shadow-xl lg:bg-slate-100`}` : ``}`}
                                onClick={() => {
                                    if (cookie) {
                                        ...
                                        global.analytics.track("User is checking room", {
                                           action: "User is checking room",
                                           room:room?.room_name,
                                           user: user.user,
                                           time: Date()
                                        });
                                ...
                                showRoom.visible === 1 && showRoom.index === index ?
                                    <div className=" md:-mx-auto pt-5 mb-10 rounded shadow-lg bg-slate-100 md:hidden lg:hidden">
                                        <div className="flex justify-between px-5">
                                            ...
                                            {room?.unconditional_rates?.filter(i=>i.meal_name=='Room Only').map((resource, index) => {
                                                return <p key={index} className="text-sm mt-2 text-gray-500 font-medium">{resource?.price} {currency}</p>
                                            })}
                                        </div>
